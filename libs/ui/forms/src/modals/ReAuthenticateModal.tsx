...
import { firebase, FirebaseAuth, parseFirebaseError } from '@omagize/api';
import { PasswordInput } from '@omagize/ui/components';
...
export function ReAuthentricateModal({
  target,
  onClose,
}: {
  onClose: () => void;
...
function Email({ onSuccess }: ProviderProps) {
  ...
  const mutation = useMutation((pw: string) => FirebaseAuth.reauth.withPassword(pw), {
    onSuccess,
  });
  return (
